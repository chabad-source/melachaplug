...
namespace LocationExtras  {
    ...
    void onBoot() {
        ...
        // } else 
        if (id(lat_global) == 40.669010f && id(lon_global) == -73.942871f) {
            ...
            if (id(lat_global) == 40.668999f && id(lon_global) == -73.942871f) {
                id(auto_location_found).publish_state("Using preprogrammed location");
                ESP_LOGD("onBoot", "Using preprogrammed location");
            } else {
    ...
    void revertToDefaultLocation() {
        // reverts to default location (we change the last two digits so it shouldn't trigger the auto location detection feature)
        id(lat_text_sensor).publish_state("40.668999");
        id(lon_text_sensor).publish_state("-73.942871");

        // int separator = id(lat_and_lon_global).find(",");
        // id(lat_text_sensor).publish_state( id(lat_and_lon_global).substr(0, separator -2) );
        // id(lon_text_sensor).publish_state( id(lat_and_lon_global).substr( separator +1, id(lat_and_lon_global).length() ) );
        id(auto_location_found).publish_state("Using preprogrammed location");
    }
